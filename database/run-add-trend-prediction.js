...
async function runSQL() {
    try {
        console.log('🚀 开始添加股票趋势预测提示词模板...');

        // 读取SQL文件
        const sqlPath = path.join(__dirname, 'add-trend-prediction-prompt.sql');
        const sql = fs.readFileSync(sqlPath, 'utf8');

        // 执行SQL
        db.exec(sql);

        console.log('✅ 股票趋势预测提示词模板添加成功！');

        // 验证数据
        const result = db.prepare(`SELECT * FROM ai_prompt_templates WHERE scene_type = 'trend_prediction'`).get();
        if (result) {
            console.log('📊 验证数据:');
